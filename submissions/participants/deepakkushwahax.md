# deepakkushwahax - 30 Days Terraform & AWS Challenge

**Participant:** @deepakkushwahax  
**Start Date:** 2025-11-26  
**Challenge Repository:** [terraform-aws-30days](https://github.com/piyushsachdeva/Terraform-Full-Course-Aws)  
**Personal Practice Repo:** https://github.com/deepakkushwahax/Terraform/tree/main/Day-01

## Progress Overview
- **Days Completed:** 2/30
- **Current Streak:** 1 day
- **Last Submission:** 2025-11-28

---

## Day 1: Install Terraform
**Date:** 2025-11-26  
**Status:** ✅ Completed

### 📝 Blog Post
[Day 1 - Install Terraform](https://medium.com/@deepakkushwaha7523/day-01-getting-started-with-terraform-and-infrastructure-as-code-5b41578532c6?postPublishedType=initial)

### 🔗 Links
- **Social Media:** [Post](https://www.linkedin.com/feed/update/urn:li:activity:7399396816502325248/?originTrackingId=Ng1qD3rlGey02YDrHgjAqw%3D%3D)
- **Code Repository:** [GitHub](https://github.com/deepakkushwahax/Terraform/tree/main/Day-01)
- **Issue:** [#99](https://github.com/piyushsachdeva/Terraform-Full-Course-Aws/issues/99)

### 🎯 Key Learnings

1. What is Terraform & IaC?
2. Core Concepts
✔ Declarative — Tell Terraform what you want, not how
✔ Idempotent — Run it 10 times, result remains consistent
✔ Version-controlled — Your infra is now part of Git, with history & collaboration
3. Why Terraform Wins
⚡ Fast provisioning
🔁 Consistent environments
🤝 Team collaboration
🗺 State tracking
🌎 Same workflow across all cloud providers
4. The Workflow: Write → Plan → Apply

---

## Day 4: creating the s3 bucket by .tf and enable the Encryption and stored the statefile.
**Date:** 2025-11-28  
**Status:** ✅ Completed

### 📝 Blog Post
[Day 4 - creating the s3 bucket by .tf and enable the Encryption and stored the statefile.](https://medium.com/@deepakkushwaha7523/day-4-configuring-a-remote-backend-in-terraform-using-aws-s3-928f6b12b60b)

### 🔗 Links
- **Social Media:** [Post](https://www.linkedin.com/feed/update/urn:li:share:7400263323151347712/)
- **Code Repository:** [GitHub](https://github.com/deepakkushwahax/Terraform/tree/main/Day-04)
- **Issue:** [#226](https://github.com/piyushsachdeva/Terraform-Full-Course-Aws/issues/226)

### 🎯 Key Learnings

<img width=1620 height=758 alt=Image src=https://github.com/user-attachments/assets/2bc22c77-1deb-4217-88b2-b93a5f3ffd98 />

---

